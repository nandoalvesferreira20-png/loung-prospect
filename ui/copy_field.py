...
from ui.components import button_options
...
def add_copy_field(parent, label, value):
    ...
    ctk.CTkLabel(row, text=label, font=TYPOGRAPHY["caption"], text_color=COLORS["muted"]).grid(row=0, column=0, sticky="w")
    entry = ctk.CTkEntry(row, fg_color=COLORS["card"], text_color=COLORS["text"],
                         border_color=COLORS["border"], border_width=0, height=34, font=TYPOGRAPHY["body"])
    ...
    # CTk does not expose Tk readonlybackground; keep this workaround local.
    entry._entry.configure(readonlybackground=COLORS["card"], selectbackground=COLORS["selected"], selectforeground=COLORS["text"])
    entry.grid(row=1, column=0, sticky="ew")
    entry.bind("<Control-c>", lambda event: copy_selection(entry))
    entry.bind("<Control-C>", lambda event: copy_selection(entry))
    button = ctk.CTkButton(row, text="Copiar", width=90, **button_options("ghost"),
